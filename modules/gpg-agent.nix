  ...
  config = mkIf cfg.enable (mkMerge [
    ...
    (mkMerge [
      ...
      (mkIf pkgs.stdenv.isDarwin {
        launchd.agents.gpg-agent = {
          ...
          config = {
            ...
            ProgramArguments = [
              ...
              "-c"
              "exec ${toString (
                pkgs.writeShellScript "gpg-agent" ''
                  export GNUPGHOME=${homedir}
                  unset SSH_AGENT_PID
                  if [ "''${gnupg_SSH_AUTH_SOCK_by:-0}" -ne $$ ]; then
                    export SSH_AUTH_SOCK="$(${gpgPkg}/bin/gpgconf --list-dirs agent-ssh-socket)"
                  fi
                  ${gpgPkg}/bin/gpg-agent ${optionalString cfg.verbose "--verbose"} \
                  --homedir "${homedir}" --use-standard-socket --daemon
                ''
              )}"
            ];
        ...
        # ensure gpg-agent agent runs on home-manager activation
        home.activation = {
          gpg-agent =
            let
            ...
            ''
              ...
              # /bin/launchctl kickstart -k ${domain-target}/org.nix-community.home.gpg-agent
