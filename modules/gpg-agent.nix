      ...
      (mkIf pkgs.stdenv.isDarwin {
        launchd.agents.gpg-agent = {
          ...
          config = {
            ...
            Program = toString (pkgs.writeScript "gpg-agent" ''
              export GNUPGHOME=${homedir}
              ${gpgPkg}/bin/gpgconf --kill gpg-agent
              ${gpgPkg}/bin/gpgconf ${optionalString cfg.verbose "--verbose"} --launch gpg-agent
            '');
            KeepAlive = false;
            RunAtLoad = cfg.enableSshSupport;
            StandardOutPath = "${config.home.homeDirectory}/Library/Logs/GpgAgent/stdout";
            StandardErrorPath = "${config.home.homeDirectory}/Library/Logs/GpgAgent/stderr";
            Sockets = {
        ...
        # [re]load gpg-agent on home-manager activation
        home.activation = {
          gpg-agent =
            let domain-target = "gui/$(id -u ${config.home.username})";
            in ''
              /bin/launchctl bootout ${domain-target}/org.nix-community.home.gpg-agent || true
              /bin/launchctl bootstrap ${domain-target} ${config.home.homeDirectory}/Library/LaunchAgents/org.nix-community.home.gpg-agent.plist
            '';
        };
      })
    ])
