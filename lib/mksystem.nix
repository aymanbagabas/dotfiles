...
{ system, hostname, user ? "ayman", useGlobalPkgs ? true, useUserPkgs ? true
, isHeadless ? false }:
...
let
  ...
  systemFunc =
    if isDarwin then inputs.darwin.lib.darwinSystem else lib.nixosSystem;
  home-manager = if isDarwin then
