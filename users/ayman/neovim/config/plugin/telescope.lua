...
vim.keymap.set('n', "<leader>fR", function() builtin.oldfiles({ cwd = vim.loop.cwd() }) end, { desc = "Recent (cwd)" })
-- git
...
vim.keymap.set('n', "<leader>sG", function() builtin.live_grep({ cwd = false }) end, { desc = "Grep (cwd)" })
...
vim.keymap.set('v', "<leader>sW", function() builtin.grep_string({ cwd = false }) end, { desc = "Selection (cwd)" })
